...
const VendorDashboard = ({ setActivePage }) => {
	...
	const fetchServices = useCallback(async () => {
		...
		try {
			...
			const servicesData = Array.isArray(result.data)
				? result.data
	...
	const calculateAnalyticsData = useCallback(
		(analyticsData, bookings, reportData) => {
			...
			const finalTotalReviews =
				reportData?.totalReviews || reviews.length;
	...
	useEffect(() => {
		...
		const loadData = async () => {
			...
			try {
				...
				const [bookingsData, analyticsData, servicesData, reportData] =
				...
				const processedAnalytics = calculateAnalyticsData(
					analyticsRaw,
					bookings,
					report
				);
	...
	const bookingStats = React.useMemo(() => {
		...
		const confirmed = vendorBookings.filter(
			(booking) =>
				booking.status === "confirmed" || booking.status === "accepted"
		).length;
		const pending = vendorBookings.filter(
			(booking) => booking.status === "pending"
		).length;
		const rejected = vendorBookings.filter(
			(booking) =>
	...
	const recentBookings = React.useMemo(() => {
		...
		return vendorBookings.slice(0, 3).map((booking) => ({
			...
			amount: booking.budget
				? `R${Number(booking.budget).toLocaleString()}`
				: "R0",
		}));
	...
	const recentReviews = React.useMemo(() => {
		...
		return analytics.reviews.map((review) => ({
			...
			date: convertFirebaseTimestamp(
				review.timeOfReview || review.createdAt
			),
		}));
		...
		if (!analytics?.ratingDistribution || analytics.totalReviews === 0) {
			return (
				<div className="no-distribution-data">
					...
					<small>
						Distribution will appear as customers leave reviews
					</small>
				</div>
			);
		}
