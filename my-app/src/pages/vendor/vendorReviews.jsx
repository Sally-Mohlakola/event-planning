	...
	<div className="star-rating">
		{[1, 2, 3, 4, 5].map((i) => {
			if (i <= Math.floor(rating))
				...
				return <StarHalf key={i} size={size} color="#fbbf24" />;
...
const VendorReviews = () => {
	...
	const convertFirebaseTimestamp = useCallback((timestamp) => {
		...
		try {
			...
			console.error(
				"Error converting Firebase timestamp:",
				error,
				timestamp
			);
			return "Recently";
	...
	const handleReply = async (reviewId, replyText) => {
		...
		try {
			...
			const res = await fetch(
				`${BASE_URL}/analytics/${vendorId}/reviews/${reviewId}/reply`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
	...
	const ratingCounts = {
		...
		good: reviews.filter(
			(r) => (r.rating || 0) >= 3.5 && (r.rating || 0) < 4.5
		).length,
		average: reviews.filter(
			(r) => (r.rating || 0) >= 2.5 && (r.rating || 0) < 3.5
		).length,
					...
					<div key={key} className="rating-bar">
						...
						<div className="rating-progress">
							<div
								...
								style={{
									width: `${
										(ratingCounts[key] / totalReviews) * 100
									}%`,
								}}
							></div>
						</div>
