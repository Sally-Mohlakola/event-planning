    ...
    >
      ...
      >
        ...
        <section className="delete-confirmation">
          <p>
            Are you sure you want to delete the task "{taskToDelete}"? This
...
const onTaskAdd = async (taskInfo) => {
  const currentTasks = Array.isArray(eventData.tasks) ? eventData.tasks : [];
    const updatedEventData = {
    ...eventData,
    tasks: [...currentTasks, { taskName: taskInfo.taskName, completed: false }],
    };
...
const onTaskToggle = async (taskName) => {
  const updatedTasks = eventData.tasks.map(task =>
    task.taskName === taskName ? { ...task, completed: !task.completed } : task
  );
  const updatedEventData = { ...eventData, tasks: updatedTasks };
  setEventData(updatedEventData);
  await saveTasks(updatedEventData);
...
const onTaskDelete = async (taskName) => {
  const updatedTasks = eventData.tasks.filter(task => task.taskName !== taskName);
  const updatedEventData = { ...eventData, tasks: updatedTasks };
  setEventData(updatedEventData);
  await saveTasks(updatedEventData);
};
  ...
  return (
    <section className="tasks-content">
      <section className="tasks-header">
        <h3>Event Tasks</h3>
        <button className="task-btn" onClick={() => setShowAddTaskPopup(true)}>
          + Add Task
        </button>
      </section>
      ...
      <section className="tasks-list">
        {eventData.tasks && eventData.tasks.length > 0 ? (
            ...
            <section className="empty-state">
                <p>No tasks added yet. Click "Add Task" to start organizing your eventData planning.</p>
            </section>
            )}
      </section>
